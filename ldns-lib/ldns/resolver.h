 ...
 */
ldns_pkt* ldns_resolver_query(const ldns_resolver *r, const ldns_rdf *name, ldns_rr_type type, ldns_rr_class c, uint16_t flags);
...
/**
 * Create a resolver structure from system default nameservers.
 * On *NIX systems this will be from the file /etc/resolv.conf. 
 * On Windows registry entries are used. 
 * \param[out] r the new resolver
 * \return LDNS_STATUS_OK or the error
 */
ldns_status ldns_resolver_new_default(ldns_resolver **r);
